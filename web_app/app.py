...
chatbot_pronto = False # We will set this to True manually for now
...
# verificar_e_processar_dados() # COMMENTED OUT
...
# Manually set chatbot_pronto to True for testing
chatbot_pronto = True
...
def chat():
    ...
    # Since chatbot_pronto is True, this will now try to call the actual get_chatbot_answer_stream
    # We need to mock this as well, or the app will crash if the import failed.
    # For now, let's make it return the maintenance message directly.
    def get_chatbot_answer_stream(pergunta):
        yield "data: " + json.dumps({"answer": "Serviço em manutenção (temporariamente desativado para depuração)."}}) + "\n\n"
